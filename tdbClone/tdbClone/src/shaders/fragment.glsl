...
vec3 palette( float t ) {
    vec3 color_a = vec3(0.44, 0.0, 0.89);
    vec3 color_b = vec3(0.64, 0.36, 0.91);
    vec3 color_c = vec3(0.0, 1.0, 0.12);

    return color_a + color_b*cos(6.28318*(t * color_c));
...
float clouds(vec2 uv) {
    ...
    float scale5 = 48.0;
    float scale6 = 96.0;
    return normnoise(snoise(vec3((uv+off1)*scale1,uTime*0.5))*0.8 + 
...
void main()
{
   ...
   // 1. Normalize UVs and handle aspect ratio
    vec2 uv = vUv;
    float aspect = iResolution.x / iResolution.y;
    
    // 2. Use the mouse position for the light
    // We use uMouse directly as it will be passed in 0.0 to 1.0 range
    vec2 lightPos = uMouse; 
    
    // Adjust both UV and Light for aspect ratio so the glow is a perfect circle
    uv.x *= aspect;
    lightPos.x *= aspect;

    vec3 activeColor;
    float cycle = mod(uTime * 0.1, 2.0);
    ...
    float cloudIntensity = 0.8 * (1.0 - (2.0 * dist));
    float lightIntensity = 0.00001 / dist; // The "Glow"

    // 4. Combine
    vec3 finalColor = vec3(cloudIntensity * clouds(uv)) * lightColor;
